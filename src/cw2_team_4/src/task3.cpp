...
namespace task3 {
    ...
    std::string estimateShapeSize(const std::vector<cv::Point> &contour, float scale) {
        ...
        float realArea = area / (scale * scale); // convert pixel^2 to mm^2
    ...
    bool solve(const cw2_world_spawner::Task3Service::Request &req,
        ...
        for (double y = y_min; y <= y_max; y += step_size) {
            for (double x = x_min; x <= x_max; x += step_size) {
                ...
                float scale, offsetX, offsetY;
                pointCloudToImage(cloud, 500, 500, scale, offsetX, offsetY);
        ...
        for (const auto& centroid : centroidsSquare) {
            std::cout << "Square: (" << centroid.x << ", " << centroid.y << ")\n";
        }
        for (const auto& centroid : centroidsCross) {
            std::cout << "Cross: (" << centroid.x << ", " << centroid.y << ")\n";
        }

        for (const auto& entry : shapePointClouds) {
            pcl::visualization::PCLVisualizer viewer("Object Viewer");
            viewer.addPointCloud<pcl::PointXYZRGB>(entry, "cloud");
